"""Distributed training of MobileNetV3-Large with balanced sampling for waste classification."""
...
def get_balanced_indices(dataset, rank, world_size):
    """Compute oversampled indices for balanced class distribution, sharded by rank."""
...
def train_worker(rank, world_size):
    """Single GPU training worker for distributed data-parallel training."""
    ...
    train_loader = DataLoader(
        train_subset, batch_size=BATCH_SIZE_PER_GPU, shuffle=True,
        num_workers=NUM_WORKERS, pin_memory=True,
    )

    val_sampler = DistributedSampler(val_dataset, num_replicas=world_size, rank=rank, shuffle=False)
    val_loader = DataLoader(
        val_dataset, batch_size=BATCH_SIZE_PER_GPU, sampler=val_sampler, num_workers=NUM_WORKERS,
    )
    ...
    class_counts = torch.tensor(
        [len(np.where(targets == t)[0]) for t in np.unique(targets)], dtype=torch.float
    )
    cw = 1.0 / class_counts
    ...
    for epoch in range(EPOCHS):
        # Unfreeze backbone at epoch 10 with differential learning rates
        if epoch == 10:
