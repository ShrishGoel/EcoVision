"""Real-time waste detection and sorting using ONNX inference and motor control."""
...
# ImageNet normalization constants (pre-scaled for uint8 input)
MEAN = (0.485 * 255, 0.456 * 255, 0.406 * 255)
STD = (1.0 / (0.229 * 255), 1.0 / (0.224 * 255), 1.0 / (0.225 * 255))


def load_optimized_model(model_path):
    """Load ONNX model with graph optimizations enabled."""
...
class EcoVisionTurbo:
    """Real-time waste classification pipeline with edge-triggered inference."""
    ...
    def inference_thread(self):
        """Background thread: detect object presence via edge count, then classify."""
            ...
            if self.latest_frame is not None:
                ...
                if cv2.countNonZero(edges) > EDGE_THRESHOLD:
                    if (time.time() - self.last_trigger_time) > COOLDOWN_SECONDS:
                        ...
                        # Collect multiple frames for ensemble averaging
                        ...
                        for _ in range(5):
                            if self.latest_frame is not None:
                                ...
                                current_roi = self.latest_frame[
                                    c_y - curr_min // 2:c_y + curr_min // 2,
    ...
    def process_inference(self, frames):
        """Run inference on collected frames and average predictions."""
        ...
        if avg_probs[idx] > 0.7:
            self.motors.sort(idx)

    def run(self):
        """Main loop: capture frames, display predictions, and handle shutdown."""
