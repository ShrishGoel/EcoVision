"""Segment waste objects from raw images using SAM and composite onto a clean background."""
...
# Minimum mask coverage to accept a segmentation result
MIN_MASK_PERCENT = 0.005

# Text prompts for SAM semantic segmentation, keyed by waste category
WASTE_PROMPTS = {
    "black": [
...
class SmartSegmenter:
    """Wrapper around SAM 3.0 for prompt-driven waste segmentation."""

    def __init__(self):
        log.info(f"Initializing SAM 3.0 on {DEVICE}...")
        overrides = {
...
def main():
    if not BG_IMAGE_PATH.exists():
        log.error(f"Background image not found: {BG_IMAGE_PATH}")
        return
    ...
    total = len(tasks)
    log.info(f"Processing {total} images...")
    ...
    for i, (img_p, out_p, color) in enumerate(tasks, 1):
        ...
        if mask is not None:
            ...
            # Alpha-blend segmented object onto clean background
            ...
            log.warning(f"[{i}/{total}] FAILED: no mask for {img_p.name}")
            stats["failed"] += 1
    ...
    print(f"  Excluded (small):   {stats['too_small']}")
    print(f"  Failed (no mask):   {stats['failed']}")
    print(f"  Total:              {total}")
    print(f"{'=' * 45}")
